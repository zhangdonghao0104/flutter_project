...
import 'package:flutter_zdh/page/web_page.dart';
...
import 'package:flutter_zdh/viewmodel/discovery/news_viewmodel.dart';
...
class _NewsPageState
  ...
  Widget _itemTitle(NewsItemModel model) {
    return Container(
  ...
  List<Widget> _getTitleList(List<String> titleList) {
    return titleList.map((title) {
      return Padding(
        ...
        child: Text(title,
            style: TextStyle(color: ColorUtils.desTextColor, fontSize: 12),
            // 做多显示三行
            maxLines: 3,
            // 超出的部分，省略号处理
